...
//path helpers
  ...
  inline void ensureCapacity(Path* path, size_t capacity)
  {
  ...
  inline void appendSlice(Path* path, Slice slice)
  {
  ...
  inline void pathAppendString(Path* path, const char* string)
  {
    Slice slice;
    slice.start = string;
  ...
  inline size_t absolutePrefixLength(const char* path)
  {
  #ifdef _WIN32
    // Drive letter.
  ...
  inline PathType pathType(const char* path)
  {
  ...
  inline Path* pathNew(const char* string)
  {
  ...
  inline void pathFree(Path* path)
  {
    if (path->chars) free(path->chars);
    free(path);
  }

  inline void pathDirName(Path* path)
  {
  ...
  inline void pathRemoveExtension(Path* path)
  {
    for (size_t i = path->length - 1; i < path->length; i--)
    {
  ...
  inline void pathAppendChar(Path* path, char c)
  {
  ...
  inline void pathJoin(Path* path, const char* string)
  {
  ...
  inline void pathNormalize(Path* path)
  {
  ...
  inline char* pathToString(Path* path)
  {
...
//file helpers
  ...
  // could not be read.
  inline char* readFile(const char* path)
  {
...
//VM bindings
  ...
  inline void vm_write(WrenVM* vm, const char* text)
  {
    printf("%s", text);
  }

  inline void reportError(WrenVM* vm, WrenErrorType type,
  ...
  inline char* readModule(WrenVM* vm, const char* module) {
...
//main helpers
  ...
  inline bool isModuleAnAPITest(const char* module) {
    if(strncmp(module, "test/api", 8) == 0) return true;
    if(strncmp(module, "test/benchmark", 14) == 0) return true;
    return false;
  }

  inline WrenInterpretResult runFile(WrenVM* vm, const char* path)
  {
  ...
  inline int handle_args(int argc, const char* argv[]) {
