...
// This source file is copied from CPython's dtoa.c with modifications to
// integrate with Wren.
//
// In addition to the changes listed below, the following modifications
// were added:
//
//  * All functions with the prefix _Py_dg are now prefixed with wren
 ...
 * otherwise using Python 3.9.2 software in source or binary form and its
 * associated documentation.
 * 
 * 2. Subject to the terms and conditions of this License Agreement, PSF
 * hereby grants Licensee a nonexclusive, royalty-free, world-wide license
 * to reproduce, analyze, test, perform and/or display publicly, prepare
 * derivative works, distribute, and otherwise use Python 3.9.2 alone or in
 * any derivative version, provided, however, that PSF's License Agreement
 * and PSF's notice of copyright, i.e., "Copyright © 2001-2021 Python
 * Software Foundation; All Rights Reserved" are retained in Python 3.9.2
 ...
 * PARTICULAR PURPOSE OR THAT THE USE OF PYTHON 3.9.2 WILL NOT INFRINGE ANY
 * THIRD PARTY RIGHTS.
 * 
 * 5. PSF SHALL NOT BE LIABLE TO LICENSEE OR ANY OTHER USERS OF
 * PYTHON 3.9.2 FOR ANY INCIDENTAL, SPECIAL, OR CONSEQUENTIAL DAMAGES OR
 * LOSS AS A RESULT OF MODIFYING, DISTRIBUTING, OR OTHERWISE USING
 * PYTHON 3.9.2, OR ANY DERIVATIVE THEREOF, EVEN IF ADVISED OF THE
 * POSSIBILITY THEREOF.
 * 
 ...
 * 
 * 8. By copying, installing or otherwise using Python 3.9.2, Licensee
 * agrees to be bound by the terms and conditions of this License Agreement.
 *
 ***************************************************************/

// Python's modifications:

/****************************************************************
 * This is dtoa.c by David M. Gay, downloaded from
 * http://www.netlib.org/fp/dtoa.c on April 15, 2009 and modified for
 * inclusion into the Python core by Mark E. T. Dickinson and Eric V.
 *Smith.
 *
 * Please remember to check http://www.netlib.org/fp regularly (and
 *especially before any Python release) for bugfixes and updates.
 *
 ...
 *     many of the #ifdef'd sections.  In particular, code to support VAX
 *and IBM floating-point formats, hex NaNs, hex floats, locale-aware
 ...
 *     Similarly, rv_alloc and nrv_alloc (return type char *) return NULL
 *on failure.  bigcomp now has return type int (it used to be void) and
 ...
 *     C style guide (PEP 7).
 *
 *  6. A bug in the memory allocation has been fixed: to avoid FREEing
 *memory that hasn't been MALLOC'ed, private_mem should only be used when k
 *<= Kmax.
 *
 *  7. _Py_dg_strtod has been modified so that it doesn't accept strings
...
/* The code below assumes that the endianness of integers matches the
   endianness of the two 32-bit words of a double.  Check this. */
    ...
    (defined(DOUBLE_IS_LITTLE_ENDIAN_IEEE754) ||                          \
     defined(DOUBLE_IS_ARM_MIXED_ENDIAN_IEEE754))
...
/* maximum permitted exponent value for strtod; exponents larger than
   MAX_ABS_EXP in absolute value get truncated to +-MAX_ABS_EXP.
   MAX_ABS_EXP should fit into an int. */
#ifndef MAX_ABS_EXP
#define MAX_ABS_EXP 1100000000U
#endif
/* Bound on length of pieces of input strings in _Py_dg_strtod;
   specifically, this is used to bound the total number of digits ignoring
   leading zeros and the number of digits that follow the decimal point.
   Ideally, MAX_DIGITS should satisfy MAX_DIGITS + 400 < MAX_ABS_EXP; that
   ensures that the exponent clipping in _Py_dg_strtod can't affect the
   value of the output. */
#ifndef MAX_DIGITS
#define MAX_DIGITS 1000000000U
#endif

/* Guard against trying to use the above values on unusual platforms with
 * ints of width less than 32 bits. */
...
/* struct Bigint is used to represent arbitrary-precision integers.  These
   integers are stored in sign-magnitude format, with the magnitude stored
   as an array of base 2**32 digits.  Bigints are always normalized: if x
   is a Bigint then x->wds >= 1, and either x->wds == 1 or x[wds-1] is
   nonzero.
...
/* Alternative versions of Balloc and Bfree that use PyMem_Malloc and
   PyMem_Free directly in place of the custom memory allocation scheme
   above. These are provided for the benefit of memory debugging tools like
   Valgrind. */
...
/* Multiply a Bigint b by m and add a.  Either modifies b in place and
   returns a pointer to the modified b, or Bfrees b and returns a pointer
   to a copy. On failure, return NULL.  In this case, b will have been
   already freed. */
...
/* convert a string s containing nd decimal digits (possibly containing a
   ...
   function carries on where the parsing code in _Py_dg_strtod leaves off:
   on entry, y9 contains the result of converting the first 9 digits.
...
/* shift a Bigint b left by k bits.  Return a pointer to the shifted
   result, or NULL on failure.  If the returned pointer is distinct from b
   then the original b will have been Bfree'd.   Ignores the sign of b. */
...
/* Do a three-way compare of a and b, returning -1 if a < b, 0 if a == b
   and 1 if a > b.  Ignores signs of a and b. */
...
/* Given a positive normal double x, return the difference between x and
   the next double up.  Doesn't give correct results for subnormals. */
...
/* Convert a scaled double to a Bigint plus an exponent.  Similar to d2b,
   ...
   should be either 0 or 2*P), and the normalization for the return value
   is different (see below).  On input, d should be finite and nonnegative,
   and d / 2**scale should be exactly representable as an IEEE 754 double.
...
/* special case of Bigint division.  The quotient is always in the range 0
   <= quotient < 10, and on entry the divisor S is normalized so that its
   top 4 bits (28--31) are zero and bit 27 is set. */
...
/* The bigcomp function handles some hard cases for strtod, for inputs
   ...
   In the following, write dv for the absolute value of the number
   ...
   Inputs:
     ...
     bc is a struct containing information gathered during the parsing and
        ...
        bc->e0 gives the exponent of the input value, such that dv =
   (integer given by the bd->nd digits of s0) * 10**e0
        ...
        bc->scale is the value used to scale rv to avoid doing arithmetic
   with subnormal values.  It's either 0 or 2*P (=106).

   Outputs:

     On successful exit, rv/2^(bc->scale) is the closest double to dv.

     Returns 0 on success, -1 on failure (e.g., due to a failed malloc
   call). */
...
static int bigcomp(U *rv, const char *s0, BCinfo *bc) {
    ...
    /* record whether the lsb of rv/2^(bc->scale) is odd:  in the exact
       halfway case, this is used for round to even. */
    odd = b->x[0] & 1;
     ...
     * == b/d, or s0 > b/d.  Here the digits of s0 are thought of as
     * representing a number in the range [0.1, 1). */
    if (cmp(b, d) >= 0)
        /* b/d >= 1 */
        dd = -1;
    else {
...
double wrenStrtod(const char *s00, char **se) {
     ...
     *    s0[0:nd0] and s0[nd0+1:nd+1] using the usual Python half-open
     * slice notation.  (If nd0 < nd, then s0[nd0] contains a '.'
     * character; if nd0 == nd, then s0[nd0] could be any non-digit
     * character.)
     *
     ...
     *    with indices in [9, min(16, nd)).  So y * 10**(min(16, nd) - 9) +
     * z gives the value represented by the first min(16, nd) sig. digits.
     */
        ...
        if (e1 >>= 4) {
            ...
            for (j = 0; e1 > 0; j++, e1 >>= 1)
                if (e1 & 1)
                    dval(&rv) *= tinytens[j];
            if (bc.scale &&
                (j = 2 * P + 1 - ((word0(&rv) & Exp_mask) >> Exp_shift)) >
                    0) {
    ...
    if (nd > STRTOD_DIGLIM) {
        ...
        /* Truncate input to 18 significant digits, then discard any
           trailing zeros on the result by updating nd, nd0, e and y
           suitably. (There's no need to update z; it's not reused beyond
           this point.) */
        for (i = 18; i > 0;) {
            /* scan back until we hit a nonzero digit.  significant digit
            'i' is s0[i] if i < nd0, s0[i+1] if i >= nd0. */
            --i;
            if (s0[i < nd0 ? i : i + 1] != '0') {
                ++i;
                break;
    ...
    /* Notation for the comments below.  Write:
         ...
         - dv for the absolute value of the number represented by the
       original decimal input string.
         ...
         - srv for the quantity rv/2^bc.scale; so srv is the current binary
           approximation to tdv (and dv).  It should be exactly
       representable in an IEEE 754 double.
    */
    ...
    for (;;) {
        ...
        /* This is the main correction loop for _Py_dg_strtod.
           ...
           We've got a decimal value tdv, and a floating-point
           approximation srv=rv/2^bc.scale to tdv.  The aim is to determine
           whether srv is close enough (i.e., within 0.5 ulps) to tdv, and
           to compute a new approximation if not.

           To determine whether srv is close enough to tdv, compute
           integers bd, bb and bs proportional to tdv, srv and 0.5 ulp(srv)
           respectively, and then use integer arithmetic to determine
           whether |tdv - srv| is less than, equal to, or greater than 0.5
           ulp(srv).
        */
        ...
        /* At this stage bd5 - bb5 == e == bd2 - bb2 + bbe, bb2 - bs2 == 1,
           and bs == 1, so:
              ...
              tdv == bd * 10**e = bd * 2**(bbe - bb2 + bd2) * 5**(bd5 -
           bb5) srv == bb * 2**bbe = bb * 2**(bbe - bb2 + bb2) 0.5 ulp(srv)
           == 2**(bbe-1) = bs * 2**(bbe - bb2 + bs2)
           ...
           for some constant M.  (Actually, M == 2**(bb2 - bbe) * 5**bb5,
           but this fact is not needed below.)
        */
        ...
        /* Now bd, bb and bs are scaled versions of tdv, srv and 0.5
           ulp(srv), respectively.  Compute the difference |tdv - srv|, and
           compare with 0.5 ulp(srv). */
        ...
        if (bc.nd > nd && i <= 0) {
            ...
            /* Here rv overestimates the truncated decimal value by at most
               ...
               Exception: if scaled rv is a normal exact power of 2, but
               not DBL_MIN, then rv - 0.5 ulp(rv) takes us all the way down
               to the next double, so the correctly rounded result is
               either rv - 0.5
            ...
            if (!word1(&rv) && !(word0(&rv) & Bndry_mask)) {
                ...
                /* rv / 2^bc.scale = 2^(j - 1023 - bc.scale); use bigcomp
                   if rv / 2^bc.scale >= 2^-1021. */
                if (j - bc.scale >= 2) {
